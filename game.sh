...
# The game definations
guess() {
	local -i mynumber=$((1 + $RANDOM % 10))
	read -p "I'm thinking of a number between 1 and 10. What do you think it is? " th$
...
# If there's an arguments provided, jump right to that game
case $1 in 
	"guess") guess;;
	"flip") flip;;
	"dice") dice;;
